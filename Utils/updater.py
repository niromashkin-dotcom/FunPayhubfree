...
def get_tags(current_tag: str) -> list[str] | None:
    ...
    try:
        ...
        while not any([el.get("name") == current_tag for el in json_response]):
            if page != 1:
                time.sleep(1)
            data = _http_client.get(f"https://api.github.com/repos/sidor0912/FunPayHub/tags?page={page}",
                                    headers=HEADERS)
            if not data:
                logger.debug("Update returned empty data!")
                return None
            else:
                json_response.extend(data if isinstance(data, list) else [data])
                page += 1
        tags = [i.get("name") for i in json_response]
        return tags or None
    except HTTPClientError as e:
        logger.debug(f"Update status code is HTTP {e.status_code}!")
        return None
    except:
...
def get_releases(from_tag: str) -> list[Release] | None:
    ...
    try:
        ...
        while not any([el.get("tag_name") == from_tag for el in json_response]):
            if page != 1:
                time.sleep(1)
            data = _http_client.get(f"https://api.github.com/repos/sidor0912/FunPayHub/releases?page={page}",
                                    headers=HEADERS)
            if not data:
                logger.debug("Update returned empty data!")
                return None
...
def download_zip(url: str) -> int:
    ...
    """
    try:
        with _http_client._session.get(url, stream=True) as r:
            r.raise_for_status()
